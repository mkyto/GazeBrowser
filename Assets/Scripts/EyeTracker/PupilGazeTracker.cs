...
public class PupilGazeTracker:MonoBehaviour
{
	...
	bool _isDone=false;
    Pupil.PupilData3D _pupilData = new Pupil.PupilData3D();
    ...
    public bool Head = false; //head conditions, eye conditions
    ...
    public bool m_DoCalibration = false;
    public float m_ConfidenceLevel = 0.5f;
	...
	void NetMQClient()
	{
		...
		if (_isconnected)
		{
            ...
            if(m_DoCalibration)
            {
                subscriberSocket.Subscribe("pupil.0"); //subscribe for gaze data
                //subscriberSocket.Subscribe("pupil.1");
            }
            else
            {
			...
			while ( _isDone == false)
			{
				...
				if (_isconnected)
				{
					try
					{
                        ...
                        if(m_DoCalibration)
                        {
                            if(msgType=="pupil.0" /*|| msgType == "pupil.1"*/ )
                            {
                                ...
                                lock (_dataLock)
                                {
                                    ...
                                    //Debug.Log("Pupil ID:"+_pupilData.id);
                                    if (_pupilData.confidence > m_ConfidenceLevel)
                                    {
                                        OnPacket(_pupilData);
                                    }
                                }
	...
	public void StartCalibration()
	{
        ...
        /*	StartCalibration (new Vector2[] {new Vector2 (0.5f, 0.5f), new Vector2 (0f, 0f), new Vector2 (0f, 0.5f),
            ...
            */
        //StartCalibration(new Vector2[] {new Vector2 (0.5f, 0.5f), new Vector2 (0.1f, 0.1f), new Vector2 (0.1f, 0.5f),
	...
	void OnPacket(Pupil.PupilData3D data)
	{
     ...
     //   Debug.Log("pupil ID:"+data.id);
		...
		if (m_status == EStatus.ProcessingGaze) { //gaze processing stage
			...
			if (data.id == 0) {
				leftEye.AddGaze (x, y);
                //Debug.Log("left add gaze");
				if (OnEyeGaze != null)
					OnEyeGaze (this);
			} else if (data.id == 1) {
				rightEye.AddGaze (x, y);
                //Debug.Log("right add gaze");
				if (OnEyeGaze != null)
					OnEyeGaze (this);
			}
